...
from common.Helpers.os_helpers import is_process_running, launch_notif_server, launch_log_server, launch_telecommand_server
...
class DEFAULT_LOGGER_FORMAT:
    LOG="%(asctime)s.%(msecs)03d %(levelname)-8s %(filename)-23s %(funcName)-19s %(lineno)-4s  %(message)s"
    DATEFMT="%Y-%m-%d %H:%M:%S"
    LOG_SERVER="%(asctime)s.%(msecs)03d %(name)-23s %(levelname)-8s %(filename)-23s %(funcName)-19s %(lineno)-4s  %(message)s"
...
class MyLogger:
    ...
    def __init__(self, name, config:Config, enable_notifications:bool=True, log_server:bool=True, server_notification:bool=True, \
        ...
        logging.basicConfig(
        handlers=[logging.StreamHandler(), logging.handlers.SocketHandler("127.0.0.1", logging.handlers.DEFAULT_TCP_LOGGING_PORT)],                 
        encoding="utf-8", 
        ...
        if enable_notifications:
            if server_notification:
                ...
                if not is_process_running(notif_server):
                    ...
                    _ = launch_notif_server(osGetcwd(), self.config.COMMON_FILE_PATH, self.config.NT_PORT)
        ...
        if start_telecommand and not DEBUG:
            ...
            if not is_process_running(tele_remote):
                ...
                _ = launch_telecommand_server(osGetcwd(), self.config.COMMON_FILE_PATH, self.config.TB_PORT)
