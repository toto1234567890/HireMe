...
DEFAULT_WAIT_RATIO = 3 
# multiply by heart beat
...
class StarQs(Telecommand, metaclass=Singleton):
    ...
    def starte(self):
        if not self.run:
            ...
            self.logger.error("{0} : StarQs, received 'start' message but it's already running !".format(self.Name))
            return True
    ...
    def append_paired(self, queueName:str, subscriber):
        ...
        self.logger.info("{0} : StarQs, has subscribe to Threads Queues '{1}' !".format(queueName, self.Name))
            ...
            while self.run:
                for key, Que in self.SafeQ.items():
                    if len(Que) > 0:
                        if self.StarQlocked:
                            while True:
                                self.logger.debug("{0} : StarQs, received lock signal, waiting '{1}s' before reading 'in' queue...".format(self.Name, self.main_queue_beat))
                        ...
                        try :
                            cur_msg = Que.popleft()
                            self.logger.debug("{0} : StarQs, message id '{1}' received from '{2}' to '{3}'".format(self.Name, cur_msg.id, key, cur_msg.too))
                            self.Route_msg(cur_msg, self.QueuesOut)#, logger=self.logger)
                        except Exception as e:
                            self.logger.critical("{0} : StarQs, critical error while trying to get message id '{1}' from Threads Queues '{2}' : {3}".format(self.Name, getOrDefault(cur_msg.id, "#None"), Que, e))
                            self.SafeQ = self.QueuesIn.copy()
                            LostQmsg(cur_msg)
                            pass
                    sleep(self.main_queue_beat)
        except Exception as e:
            self.logger.critical("{0} : StarQs, critical error in main loop '{0}'".format(self.Name, e))
            exit(1)

        self.logger.info("{0} : StarQs, main queue has been stopped.".format(self.Name))
...
class SubsQ:
                ...
                try:  
                    if data != '0':
                        if isinstance(data, Qmsg):
                            ...
                            self.logger.debug("Thread '{0}' received unknown message from Process '{0}' : {1}".format(self.name, data))
                            LostQmsg(cur_msg)
                    else:
                        self.logger.debug("Thread '{0}' received unsubscribe message from Process !".format(self.name))
    ...
    def send_msg_in(self, msg, too:str=None, ackw:bool=False):
        ...
        self.logger.debug("Thread '{0}' sent message id '{1}' to '{2}'".format(self.name, msg.id, msg.too))
        ...
        while self.run and self.router_run:
            ...
            if len(Que) > 0:
                if not self.locked:
                    try :
                        cur_msg = Que.popleft()
                        self.logger.debug("Thread '{0}' received message id '{1}' from '{2}'".format(self.name, cur_msg.id, cur_msg.frome))
                        if cur_msg.ackw:
                            self.logger.debug("Thread '{0}' send back acknow id '{1}' to '{2}'...".format(self.name, cur_msg.id, cur_msg.frome))
