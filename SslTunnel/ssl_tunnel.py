...
from common.ThreadQs.thread_Qs import SubsQ, Qmsg
...
class SslTunnel(SubsQ):
    ...
    def __init__(self, StarQ, logger, config, name:str=None, sandbox:bool=True, default_recv:str=None):
            ...
            with context.wrap_socket(sock=Sock) as SslSock: 
                ...
                while self.run and self.router_run:
                    if len(self.que) > 0:
                        ...
                        SslSock.sendall(msg)
                        self.logger.debug("{0} : message sent on SSL socket {1} to '{2}' : '{3}'".format(self.Name, self.id, self.broker_config["BR_URI"], msg))

                        data = SslSock.recv(1024)
                        if not data:
                            break
                        self.logger.debug("{0} : received message from {1}:{2} : '{3}'".format(self.Name, self.broker_config["BR_URI"], self.broker_config["BR_PORT"], data))
                        self.treat_ext_msg(data.decode())

        self.logger.debug("{0} : '{1}' connection to '{2}' closed on destport '{3}' srcport '{4}'".format(self.Name, SslSock.version(), self.broker_config["BR_URI"], self.broker_config["BR_PORT"], self.id))
