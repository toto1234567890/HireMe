...
class Database:
    ...
    @TradeToDB
    def db_order(self, broker: String, ticker: String, asset_type: String, order_type: String, buy_or_sell: String, state: String, price: float, quantity: float, amount: float, order_date: DateTime, user: String=None):
        session: Session
        with self.db_session() as session:
            session.add(Order(broker, ticker, asset_type, order_type, buy_or_sell, state, price, float(quantity), float(amount), order_date, (self.user if user == None else user)))
