...
async def __retry_internal(f, exceptions=Exception, tries=-1, delay=0, max_delay=None, backoff=1, jitter=0, logger=None):
    """
    ...
    while _tries:
        try:
            ...
            if not _tries:
                raise
            if not logger is None:
                await logger.asyncWarning("{0}, retrying in {1} seconds...".format(e, _delay))
            await asyncSleep(_delay)
            _delay *= backoff
...
def asyncRetry(exceptions=Exception, tries=-1, delay=0, max_delay=None, backoff=1, jitter=0, logger=None):
    ...
    def retry_decorator(f):
        ...
        async def retry_wrapper(*args, **kwargs):
            if f.__name__ == "watch_broker": return await __retry_internal(partial(f, *args, **kwargs), exceptions, tries, delay, max_delay, backoff, jitter, logger=kwargs["logger"])
