...
class Job:
    ...
    def __init__(self, jobName, interval):
        ...
        self.function = None ; self.args = None ; self.kwargs = None
    ...
    def __call__(self, function, wait_end=False, on_time=False, rerun_on_error=False, *args, **kwargs):
        global runLoop
        self.jobName = "{0} {1} {2}".format(function.__name__, self.jobName, uuid1())
        self.function = function
        self.args = args or []
        self.kwargs = kwargs or {}
        self.waitEnd = wait_end

        asyncJob = iscoroutinefunction(function)
    ...
    def do(function, wait_end=False, on_time=False, rerun_on_error=False, *args, **kwargs):
        pass
...
class ScheduledJobs:
    ...
    Name = "ScheduledJob"
    jobs = {}
    def __init__(self, config:Config, logger:MyLogger, name:str=None, asyncioLoop=None, mainProcess=False):
        if not name is None:
            self.Name = name
        self.is_running = False
...
if __name__ == "__main__":
    ...
    #asyncJob = inspectIscoroutine(asyncFunc)
    #for job in to_gather:
    #    job.stop()
    #asyncioGather(*to_gather, return_exceptions=True)
...
# with main Process 
    ...
    schedule.every(unit=0.01, timelaps='milliseconds').do(function=syncFunc)
    schedule.every(unit=0.01, timelaps='milliseconds').do(function=syncFunc, rerun_on_error=True)
    schedule.every(unit=5, timelaps='milliseconds').do(function=syncFunc, rerun_on_error=True, on_time=True, wait_end=True)
    schedule.every(unit=4, timelaps='milliseconds').do(function=syncFunc, rerun_on_error=True, on_time=True)
    schedule.every(unit=0.01, timelaps='milliseconds').do(function=syncFunc, rerun_on_error=True, wait_end=True)
    schedule.every(unit=4, timelaps='milliseconds').do(function=syncFunc, on_time=True, wait_end=True)
    schedule.every(unit=5, timelaps='milliseconds').do(function=syncFunc, on_time=True)
    schedule.every(unit=0.01, timelaps='milliseconds').do(function=syncFunc, wait_end=True)

    ## 2- async function only
    #schedule.every(unit=0.1, timelaps='second').do(function=asyncFunc)
    #schedule.every(unit=0.01, timelaps='milliseconds').do(function=asyncFunc, rerun_on_error=True)
    schedule.every(unit=0.1, timelaps='milliseconds').do(function=asyncFunc, rerun_on_error=True, on_time=True, wait_end=True)
    #schedule.every(unit=0.5, timelaps='second').do(function=asyncFunc, rerun_on_error=True, on_time=True)
    schedule.every(unit=3, timelaps='seconds').do(function=asyncFunc, rerun_on_error=True, wait_end=True)
    schedule.every(unit=4, timelaps='millisecond').do(function=asyncFunc, on_time=True, wait_end=True)
    #schedule.every(unit=0.5, timelaps='second').do(function=asyncFunc, on_time=True)
    schedule.every(unit=10, timelaps='milliseconds').do(function=asyncFunc, wait_end=True)
