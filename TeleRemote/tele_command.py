...
from os import getppid as osGetppid, name as osName
...
from asyncio import Lock as asyncioLock, sleep as asyncSleep, Queue as asyncioQueue, all_tasks as asyncioAll_tasks, \
                    start_server as asyncioStart_server, get_event_loop as asyncioGet_event_loop, wait as asyncioWait
...
class Telecommand:
    ...
    def stopTele(self, TeleSocket):
        if self.run:
            ...
            self.logger.error("StarQs : received 'stop' message but it's already stopped !")
            return True
